...
def dump_msr_state_to_file(dump_fname, ssh_conn, shared_names, restore=False):
    """
    ...
    _, stdout_debug, stderr_debug = ssh_conn.execute_command("free -h -m; vmstate -s")
    print("Free output after")
    print("{}".format(stdout_debug.read()))
    with open(dump_fname, "w", encoding="UTF-8") as file:
        file.write(stdout.read())

    if restore == True:
        print("^^^^")
        if os.path.exists(dump_fname) and os.stat(dump_fname).st_size != 0:
            with open(dump_fname, "r", encoding="UTF-8") as file:
                debug_data = file.readlines()
            print(f"{debug_data[-1]=}")
        print("____")
    assert stderr.read() == ""
...
def _test_cpu_wrmsr_restore(context):
    ...
    dump_msr_state_to_file(msrs_after_fname, ssh_connection, shared_names, True)
