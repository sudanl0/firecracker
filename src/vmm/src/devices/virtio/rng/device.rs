...
use super::entropy_metrics::ENTROPY_METRICS;
...
impl Entropy {
    ...
    fn handle_one(&self, iovec: &mut IoVecBufferMut) -> Result<u32, EntropyError> {
        ...
        rand::fill(&mut rand_bytes).map_err(|err| {
            ENTROPY_METRICS.host_rng_fails.inc();
            err
        })?;
    ...
    fn process_entropy_queue(&mut self) {
        ...
        while let Some(desc) = self.queues[RNG_QUEUE].pop(mem) {
            ...
            let bytes = match IoVecBufferMut::from_descriptor_chain(mem, desc) {
                Ok(mut iovec) => {
                    ...
                    if !Self::rate_limit_request(&mut self.rate_limiter, iovec.len() as u64) {
                        ...
                        ENTROPY_METRICS.entropy_rate_limiter_throttled.inc();
                    ...
                    self.handle_one(&mut iovec).unwrap_or_else(|err| {
                        error!("entropy: {err}");
                        ENTROPY_METRICS.entropy_event_fails.inc();
                        0
                    })
                }
                Err(err) => {
            ...
            match self.queues[RNG_QUEUE].add_used(mem, index, bytes) {
                Ok(_) => {
                    used_any = true;
                    ENTROPY_METRICS.entropy_bytes.add(bytes.into());
                }
                Err(err) => {
    ...
    pub(crate) fn process_entropy_queue_event(&mut self) {
        if let Err(err) = self.queue_events[RNG_QUEUE].read() {
            ...
            ENTROPY_METRICS.rate_limiter_event_count.inc();
        }
    }
...
impl VirtioDevice for Entropy {
    ...
    fn activate(&mut self, mem: GuestMemoryMmap) -> Result<(), ActivateError> {
        self.activate_event.write(1).map_err(|err| {
            ...
            ENTROPY_METRICS.activate_fails.inc();
...
mod tests {
    ...
    fn test_entropy_event() {
        ...
        let entropy_event_fails = ENTROPY_METRICS.entropy_event_fails.count();
        let entropy_event_count = ENTROPY_METRICS.entropy_event_count.count();
        let entropy_bytes = ENTROPY_METRICS.entropy_bytes.count();
        let host_rng_fails = ENTROPY_METRICS.host_rng_fails.count();
        assert_eq!(th.emulate_for_msec(100).unwrap(), 1);
        assert_eq!(
            ENTROPY_METRICS.entropy_event_fails.count(),
            entropy_event_fails + 1
        );
        assert_eq!(
            ENTROPY_METRICS.entropy_event_count.count(),
            entropy_event_count + 1
        );
        assert_eq!(ENTROPY_METRICS.entropy_bytes.count(), entropy_bytes);
        assert_eq!(ENTROPY_METRICS.host_rng_fails.count(), host_rng_fails);
        ...
        assert_eq!(ENTROPY_METRICS.entropy_bytes.count(), entropy_bytes + 30);
        assert_eq!(ENTROPY_METRICS.host_rng_fails.count(), host_rng_fails);
        ...
        assert_eq!(ENTROPY_METRICS.entropy_bytes.count(), entropy_bytes + 512);
        assert_eq!(ENTROPY_METRICS.host_rng_fails.count(), host_rng_fails);
    }
    ...
    fn test_bad_rate_limiter_event() {
        ...
        check_metric_after_block!(
            &ENTROPY_METRICS.entropy_event_fails,
            1,
    ...
    fn test_bandwidth_rate_limiter() {
        ...
        check_metric_after_block!(
            ENTROPY_METRICS.entropy_rate_limiter_throttled,
            1,
            th.device().process_entropy_queue()
        );
        assert!(th.device().rate_limiter().is_blocked());
    ...
    fn test_ops_rate_limiter() {
        ...
        check_metric_after_block!(ENTROPY_METRICS.entropy_bytes, 64, th.emulate_for_msec(100));
        assert_eq!(ENTROPY_METRICS.entropy_bytes.count(), entropy_bytes + 64);
        ...
        check_metric_after_block!(
            ENTROPY_METRICS.rate_limiter_event_count,
            1,
            th.emulate_for_msec(150)
        );
        // The rate limiter event should have processed the pending buffer as well
        assert_eq!(ENTROPY_METRICS.entropy_bytes.count(), entropy_bytes + 128);
    }
}
