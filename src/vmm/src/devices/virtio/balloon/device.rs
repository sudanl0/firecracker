...
use super::balloon_metrics::BALLOON_METRICS;
...
impl Balloon {
    ...
    pub(crate) fn process_deflate_queue(&mut self) -> Result<(), BalloonError> {
        // This is safe since we checked in the event handler that the device is activated.
        let mem = self.device_state.mem().unwrap();
        BALLOON_METRICS.deflate_count.inc();
    ...
    pub(crate) fn process_stats_queue(&mut self) -> Result<(), BalloonError> {
        // This is safe since we checked in the event handler that the device is activated.
        let mem = self.device_state.mem().unwrap();
        BALLOON_METRICS.stats_updates_count.inc();
        ...
        while let Some(head) = self.queues[STATS_INDEX].pop(mem) {
            ...
            for index in (0..head.len).step_by(SIZE_OF_STAT) {
                ...
                self.latest_stats.update_with_stat(&stat).map_err(|_| {
                    BALLOON_METRICS.stats_update_fails.inc();
...
impl VirtioDevice for Balloon {
    ...
    fn activate(&mut self, mem: GuestMemoryMmap) -> Result<(), ActivateError> {
        ...
        if self.activate_evt.write(1).is_err() {
            ...
            BALLOON_METRICS.activate_fails.inc();
...
pub(crate) mod tests {
    ...
    fn test_stats() {
        ...
        // Happy case.
        {
            ...
            check_metric_after_block!(BALLOON_METRICS.stats_updates_count, 1, {
            ...
            check_metric_after_block!(BALLOON_METRICS.event_fails, 0, {
